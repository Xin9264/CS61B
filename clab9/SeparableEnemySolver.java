...
public class SeparableEnemySolver {
    ...
    public boolean isSeparable() {
        ...
        for (String label : g.labels()){
            partition.put(label, 0);
        }
        for (String label : g.labels()){
            if (partition.get(label) == 0){
                BFS.add(label);
                partition.put(label, 1);
                while (!BFS.isEmpty()){
                    String currNode = BFS.poll();
                    for (String n : g.neighbors(currNode)){
                        if (partition.get(n) == 0){
                            BFS.add(n);
                            partition.put(n, OtherPart(partition.get(currNode)));
                        }
                        if (partition.get(n) == partition.get(currNode)) {
                            return false;
                        }
                    }
                }
